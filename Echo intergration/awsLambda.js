...
var options = {
  ...
  port: 443,
  path: '/services/[ADD UR WEBHOOK ID HERE]',
  method: 'POST'
};
...
function getWelcomeResponse(callback) {
    ...
    var speechOutput = "Welcome to the Alexa and Lambda demo app, "
                + "You can give me a message to send to our team";
...
function setMessageInSession(intent, session, callback) {
    ...
    if (messageSlot) {
        ...
        speechOutput = "Your message has been sent.";
        ...
        req.write('{"channel": "#awsiot", "username": "AWS IOT TEAM", "text": "[via Alexa]: ' + message + '", "icon_emoji": ":ghost:"}');
        req.end();
    } else {
        speechOutput = "I didn't hear your message clearly, please try again";
        repromptText = "I didn't hear your message clearly, please try again";
    callback(sessionAttributes,
             buildSpeechletResponse(cardTitle, speechOutput, repromptText, shouldEndSession));
    }
}
...
function getMessageFromSession(intent, session, callback) {
    ...
    if(message) {
        speechOutput = "Your message is " + message;
